...
from api.models import LeaveApplication

class LeaveApplicationModelTestCase(TestCase):

    def test_quick_create(self):

        user = User.objects.create(username="joe", email="joe@email.com", password="123")
        LeaveApplication.quick_create(user)

class LeaveApplicationViewSetTestCase(TestCase):

    def setUp(self):
        self.c = APIClient()
        self.user = User.objects.create(username="joe", email="joe@email.com", password="123")
        LeaveApplication.quick_create(self.user)

    def test_list_leave_applications(self):

        url = reverse('leaveapplication-list')
        response = self.c.get(url)  
        assert response.status_code == 200, \
            "Expect 200 OK. got: {}" . format (response.status_code)
        num_application = len(response.json())    
        assert num_application == 1, \
            "Expect excatly 1. Got: {}: {}" . format(num_application, response.content)
    ...
    def test_create_leave_application(self):
        ...
        assert LeaveApplication.objects.count() == 2, \
            'Expect a new LeaveApplication to have been created.'
